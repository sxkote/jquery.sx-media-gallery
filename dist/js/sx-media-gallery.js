;(function ($, window, document, undefined) {
    ...
    class SXMediaGalleryViewer {
        ...
        resize(width, height) {
            ...
            if (this.material.type == FileType.Video) {
                height -= 2 * (this.info == null || !this.info.isVisible ? 0 : this.info.element.height() || 0);
            }
        ...
        static create(width, height, infoVisible, material) {
    ...
    class SXMediaGalleryDesk {
        ...
        addMaterial(material, clickHandler) {
            ...
            if (material.avatar)
                item.append($(`<img class='${this._prefix}-item-avatar' src='${material.avatar}'>`));
    ...
    class SXMediaGalleryPanel {
        constructor(options) {
            ...
            this._element = $(`<div class='${this._prefix}'></div>`).css('display', 'none');
            this.element.ready(this.resize.bind(this));
            ...
            // create Viewer
            this._viewer = SXMediaGalleryViewer.create(this.width, this.height, this._options.infoVisible);
        ...
        addMaterial(obj) {
            ...
            if (obj instanceof SXMediaMaterial) {
                if (this.allowDuplicates || this.findMaterial(obj.url) == null) {
        ...
        resize() {
            if (this.element != null && this.viewer != null)
                this.viewer.resize(this.element.width(), this.element.height());
        }
    ...
    class SXMediaGallery {
        constructor(element, options) {
            ...
            this.panel = new SXMediaGalleryPanel(this._options);
            ...
            $(element).on('click', `.${this._prefix}-item`, function (event) {
                this.panel.show($(event.target).data('url'));
                event.stopPropagation();
            }.bind(this));

            //добавляем все HTML элементы в коллекцию материалов
            $(element).find(`.${this._prefix}-item`).each(function (index, element) {
                let url = $(element).data('url');
                let date = $(element).data('date');
                let title = $(element).data('title');
                let comment = $(element).data('comment');
                this.panel.addMaterial(new SXMediaMaterial({url: url, date: date, title: title, comment: comment}));
            }.bind(this));

            $(element).data(this._prefix, this);
        }
    }
